 ...
 * @retval  None
 **=====================================================================*/
static uint32_t Crc32BitRev(uint32_t u32InputVal, uint32_t u32Bw)  
{  
    int i;  
    uint32_t u32Val = 0;  
    uint32_t u32Input = u32InputVal;
    for(i=0;i<u32Bw;i++)  
    {  
        if(u32Input & 0x01)  
        {  
...
/**=====================================================================*
 * @brief   Crc32TableInit
 * @param   uint32_t u32Poly	
 * @retval  None
 **=====================================================================*/ 
void Crc32TableInit(uint32_t u32PolyVal)  
{  
    int i;  
    int j;  
    unsigned long c;  
    uint32_t u32Poly = u32PolyVal;
    u32Poly = Crc32BitRev(u32Poly,32);  
    for(i = 0; i < 256; i++)  
    {  
        c = i;  
        for (j = 0; j < 8; j++)  
 ...
 * @retval  None
 **=====================================================================*/ 
uint32_t Crc32Calculation( void* input, uint32_t len)  
{  
    int i;  
    unsigned char index;  
    unsigned char* pch;  
    unsigned long crc;
    // 根据用户CRC算法，可以为0，可以为0xFFFFFFF 
    crc = 0;  
    //crc = 0xffffffff;
    ...
    {  
        ...
        crc = (crc>>8)^u32aCrr32Table[index];  
        pch++;  
    }  
    // 根据用户CRC算法，可能需要异或 可以为0xFFFFFFF  
    //crc ^= 0xFFFFFFFF;  
    return crc;  
} 

// crc-16/MODBUS校验 data为输入数据，length为data数据长度
